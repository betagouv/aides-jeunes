...
const CONDITION_STATEGY = {
  ...
  quotient_familial: {
    test: (condition, { openfiscaResponse, periods }) => {
      ...
      const nbptr =
        openfiscaResponse.foyers_fiscaux._.nbptr[periods.fiscalYear.id]
      const quotient_familial = rfr / nbptr / 12
      return OPERATOR[condition.operator](quotient_familial, condition.value)
    },
    extra: [
      { id: "rfr", entity: "foyers_fiscaux", type: "float", fiscalYear: true },
      {
        id: "nbptr",
