...
var storeKey = 'situation' + window.situationId;
var storedSituation = localStorage.getItem(storeKey);

if (storedSituation) {
    situation.import(JSON.parse(storedSituation));
} else {
...
function processSituation() {
    ...
    localStorage.setItem(storeKey, JSON.stringify(situation.toJSON()));
}
