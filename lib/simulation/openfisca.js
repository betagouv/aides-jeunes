...
function mapIndividus(situation) {
    ...
    if (situation.conjoint) individus.push(situation.conjoint);
    ...
    return _.map(individus, function(individu) {
        ...
        ressources.forEach(function(ressource) {
            rs[ressource.type] = rs[ressource.type] + ressource.montant;
        });

        target.sali = 0 + rs.revenusSalarie + rs.revenusNonSalarie + rs.revenusAutoEntrepreneur;
        target.choi = 0 + rs.allocationsChomage + rs.indChomagePartiel;
        target.alr = 0 + rs.pensionsAlimentaires;
        target.rsti = rs.pensionsRetraitesRentes;
