...
function ajouteRessourceType(individu, resourceKey, openfiscaObj, openfiscaKey) {
    var resources = individu.ressourcesTroisDerniersMois();
...
function mapIndividus(demandeur) {
    ...
    return _.map(individus, function(individu) {
        ...
        var statusMarital = individu.statusMarital;
        if (dateDeNaissance) target.birth = dateDeNaissance.toString().substring(0, 10);
        if (statusMarital) target.statmarit = statmatMap[statusMarital];
        ...
        addResourceTo(individu, 'revenusSalarie', target, 'sali');
        addResourceTo(individu, 'revenusNonSalarie', target, 'sali');
        addResourceTo(individu, 'revenusAutoEntrepreneur', target, 'sali');
        addResourceTo(individu, 'allocationsChomage', target, 'choi');
        addResourceTo(individu, 'indChomagePartiel', target, 'choi');
        addResourceTo(individu, 'pensionsRetraitesRentes', target, 'rsti');
        addResourceTo(individu, 'pensionsAlimentaires', target, 'alr');
        addResourceTo(individu, 'ass', target, 'ass');
