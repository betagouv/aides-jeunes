...
export function useProgress(): ComputedRef<number> {
  ...
  const progress = computed(() => {
    ...
    const allSteps: StepLayout[] = store.getAllSteps.filter(
      (step: StepLayout) => !["/", "/simulation/resultats"].includes(step.path)
    )
    const activeSteps = allSteps.filter((step: StepLayout) => step.isActive)
    ...
    if (
      ...
      const answeredSteps: StepLayout[] = activeSteps.filter((step) =>
