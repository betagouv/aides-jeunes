...
** Configuration
*/
var OPENFISCA_API_URL = 'http://openfisca-api.herokuapp.com';
...
function ping() {
    console.log('OpenFisca: Ping!');
    request.get(OPENFISCA_API_URL + '/api/1/fields').end(function(err, response) {
        if (err) return console.log('OpenFisca: Error: ' + err);
        if (response.error) return console.log('OpenFisca: Error: ' + response.status);
        console.log('OpenFisca: Pong!');
    });
}
