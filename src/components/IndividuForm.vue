<template>
  <form @submit.prevent="submit">
    <div class="form-group" ng-class="{'has-error': submitted && form.dateDeNaissance.$invalid}">
      ...
      <div class="col-sm-4">
        <input
          ...
          max="2020-01-01"
          required
          v-model="individu.date_naissance"
          auto-focus
          aria-describedby="help-date-de-naissance"
          >
        <ul class="help-block" id="help-date-de-naissance" v-if="$v.individu.date_naissance.$dirty">
          <li v-if="!$v.individu.date_naissance.required">
    ...
    <button type="button" class="btn btn-default" ng-click="cancel()" ng-if="displayCancelButton">Annuler</button>
      -->
    <button type="button" class="btn btn-default" v-on:click="test">Test</button>
