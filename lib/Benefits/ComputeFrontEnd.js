...
const { safelyPick } = require("../SafelyPick")
...
const CONDITION_STATEGY = {
  boursier: (value, { openfiscaResponse, period }) => {
    return (
      safelyPick(
      ...
      safelyPick(
        openfiscaResponse.individus.demandeur.bourse_lycee,
        period.thisMonth.id
      ) === value
    )
  },
  mention_baccalaureat: (values, { situation }) => {
...
function testProfileConditions(conditions, data) {
  ...
  return conditions.every((condition) =>
    CONDITION_STATEGY[condition.type](condition.value || condition.values, data)
  )
}
...
function computeFrontEndBenefits(
  ...
  const period = generator(situation.dateDeValeur)
  const data = { situation, openfiscaResponse, period }
