...
function mapIndividus(situation) {
    ...
    if (situation.conjoint.birthDate) individus.push(situation.conjoint);
    if (situation.enfants) individus = individus.concat(situation.enfants);

    _.map(individus, function(individu) {
        var target = { id: individu._id };
        var dateDeNaissance = individu.dateDeNaissance;
        var statutMarital = individu.statutMarital;
        if (dateDeNaissance) target.birth = moment(dateDeNaissance).format('YYYY-MM-DD');
...
function mapFamilles(situation) {
    ...
    return [{ parents: parents, enfants: situation.enfants ? _.pluck(situation.enfants, '_id') : [] }];
}

function mapFoyersFiscaux(situation) {
    var declarants = [situation.demandeur];
    if (situation.conjoint.dateDeNaissance) declarants.push(situation.conjoint);

    declarants.forEach(function(declarant) {
        if (!declarant.dateDeNaissance || moment().diff(moment(declarant.dateDeNaissance), 'years') < 18)
            throw new Error('Declarant is under 18');
    });

    return [{
        declarants: _.pluck(declarants, '_id'),
        personnes_a_charge: _.pluck(situation.enfants, '_id')
    }];
}
