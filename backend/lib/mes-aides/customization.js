...
function determineCustomizationId(testCase, currentPeriod) {
  if (
    ...
    if (testCase.menages._.depcom[currentPeriod].match(/^13/)) return "D13-BDR"
    ...
    if (testCase.menages._.depcom[currentPeriod].match(/^57/))
      return "D57-MOSELLE"
    if (testCase.menages._.depcom[currentPeriod].match(/^59/)) return "D59-NORD"
    if (testCase.menages._.depcom[currentPeriod].match(/^62/)) return "D62-PDC"
    ...
    if (testCase.menages._.depcom[currentPeriod].match(/^83/)) return "D83-VAR"
    if (testCase.menages._.depcom[currentPeriod].match(/^92/))
      return "D92-HAUTS_DE_SEINE"
    if (testCase.menages._.depcom[currentPeriod].match(/^93/)) return "D93-SSD"
    if (testCase.menages._.depcom[currentPeriod].match(/^94/))
      return "D94-VAL_DE_MARNE"
    if (
      includes(
        lyonMetropoleInseeCodes,
        testCase.menages._.depcom[currentPeriod]
      )
    )
      return "M69-LYON"
