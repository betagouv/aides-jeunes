  ...
  <div class="container">
    ...
    <div class="form__group types" v-if="depcom">
      <div class="type" v-for="type in types" v-bind:key="type.code">
        ...
        <input type="checkbox" v-bind:name="`type-${type.code}`" v-bind:id="`type-${type.code}`" v-model="type.selected" />
      </div>
      <span v-show="updating"><i class="fa fa-spinner fa-spin" aria-hidden="true"></i> Récupération en cours…</span>
    </div>
    <div>
...
export default {
  ...
  data: function() {
    return {
      ...
      communes: [],
      types: [{
        name: 'CAF', code: 'caf', selected: true
      },{
        name: 'CPAM', code: 'cpam', selected: true
      },{
        name: 'CARSAT', code: 'carsat', selected: true
      },{
        name: 'Mairie', code: 'mairie', selected: true
      }],
      updating: false,
      places: []
    }
  ...
  computed: {
    uri: function() {
      ...
      return `https://etablissements-publics.api.gouv.fr/v3/communes/${this.depcom}/${types.join('+')}`
    },
  },
  watch: {
    codePostal: function(newCodePostal) {
      if (!newCodePostal || !newCodePostal.length || newCodePostal.length != 5) {
        return
      }

      Commune.get(newCodePostal)
      .then(l => this.communes = l)
