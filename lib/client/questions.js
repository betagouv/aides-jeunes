...
var questions = {
    Individu: {
        ...
        dateDeNaissance: {
            ...
            afterCallback: function() {
                if (this.dateDeNaissance && _.isString(this.dateDeNaissance)) {
                    this.dateDeNaissance = moment(this.dateDeNaissance, 'DD/MM/YYYY').toDate();
                }
            }
        },
        enfants: {
            ...
            afterCallback: function() {
                ...
                this.enfants.forEach(function(enfant) {
                    if (!enfant.prenom && !enfant.dateDeNaissance) return parent.retireEnfant(enfant);
                    if (_.isString(enfant.dateDeNaissance)) enfant.dateDeNaissance = moment(enfant.dateDeNaissance, 'DD/MM/YYYY').toDate();
...
var statusQuestion = {
    ...
    afterCallback: function() {
        var individu = this;
        _.forEach(situation.Individu.status, function(label, status) {
            if (!(status in individu)) individu[status] = false;
        });
    }
};
