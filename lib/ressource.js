var { categoriesRnc, ressourceTypes } = require("./constants/resources")
...
function isRessourceRelevant(ressourceType, answers, individuId) {
  return (
    !ressourceType.isRelevant || ressourceType.isRelevant(answers, individuId)
  )
}
...
function getIndividuRessourceCategories(answers, individuId) {
  ...
  return uniq(
    filter(
      ...
      (ressourceType) =>
        declaredRessources &&
        declaredRessources.includes(ressourceType.id) &&
        isRessourceOnMainScreen(ressourceType) &&
        isRessourceRelevant(ressourceType, answers, individuId),
      {}
    ).map((r) => r.category)
  )
}

function getIndividuRessourceTypes(individu, answers) {
  return filter(ressourceTypes, (ressourceType) => {
    return (
      isRessourceOnMainScreen(ressourceType) &&
      isRessourceRelevant(ressourceType, answers, individu.id)
    )
...
function getIndividuRessourceTypesByCategory(individu, category, answers) {
  return filter(ressourceTypes, (ressourceType) => {
    return (
      ressourceType.category === category &&
      isRessourceOnMainScreen(ressourceType) &&
      isRessourceRelevant(ressourceType, answers, individu)
    )
  }).reduce((accumulator, ressourceType) => {
    accumulator[ressourceType.id] = isSelectedForCurrentYear(
      individu[ressourceType.id],
