...
function mapIndividus(situation) {
    ...
    return _.map(individus, function(individu) {
        ...
        target.sali = 4 * (0 + rs.revenusSalarie + rs.revenusNonSalarie + rs.revenusAutoEntrepreneur);
        target.choi = 4 * (0 + rs.allocationsChomage + rs.indChomagePartiel);
        target.alr = 4 * (0 + rs.pensionsAlimentaires);
        target.rsti = 4 * rs.pensionsRetraitesRentes;
