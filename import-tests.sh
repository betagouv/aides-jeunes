...
read -n 1 -p "Do you want to delete the content of the local $DATABASE database as part of this import?  [y/N]" drop
...
read -n 1 -p "Do you want to LIMIT the imported situations to the one used in tests ONLY?  [y/N]" limited
...
then
    ...
    read -n 1 -p "Do you want to rename the local 'situations' collection in 'legacysituations' after the import?  [y/N]" rename
fi
