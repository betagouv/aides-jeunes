...
<template>
  <form @submit.prevent="submit">
    ...
    <p class="notification warning" v-if="isDemandeurMineur">
      ...
      Les aides simulées ne peuvent normalement être attribuées qu'à des
      personnes majeures.<br />
      Pour bénéficier d'aides, vous devrez démontrer aux organismes qui les
      distribuent que vous n'avez plus de liens avec vos parents ou tuteurs.<br />
      Si ce n'est pas le cas, faites plutôt la simulation du point de vue d'un
      de vos parents ou tuteurs.
    </p>
    ...
    <div class="form__group" v-if="captureOutOfFranceQuestions">
      <label>
        <input
          type="checkbox"
          v-model="satisfyResidentialDurationPrerequisite"
        />
        ...
        >
          Vous devez remplir cette condition pour être éligible aux aides
          sociales calculées par ce simulateur.
        </p>
      </label>
    </div>
    ...
    <div
      class="form__group"
      v-if="
        captureOutOfFranceQuestions && satisfyResidentialDurationPrerequisite
      "
    >
      <label>
        <input
    ...
    <fieldset class="form__group">
      ...
      <div>
        ...
        <label
          v-for="statut in specificSituations"
    ...
    <fieldset class="form__group" v-if="capturePerteAutonomie">
      ...
      <label v-for="gir in GIROptions" v-bind:key="gir.value">
        <input
          type="radio"
          name="gir"
          v-bind:value="gir.value"
          v-model="individu.gir"
        />
        {{ gir.label }}
      </label>
    </fieldset>
...
export default {
  ...
  computed: {
    ...
    captureOutOfFranceQuestions: function () {
      return (
        this.individu._role == "demandeur" && this.individu.nationalite != "FR"
      )
    },
